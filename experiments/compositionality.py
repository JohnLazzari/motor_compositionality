    ...
    for env in envs_to_use:
        ...
        if movement_type == "extension_retraction":
            if epoch == "stable":
                ...
                env_hs.append(
                    trial_data[mode][:, middle_movement].mean(dim=0).unsqueeze(0)
                )
            elif epoch == "retraction":
                env_hs.append(
                    trial_data[mode][:, end_movement].mean(dim=0).unsqueeze(0)
                )
            elif epoch == "hold":
...
def _two_task_pcs(model_name, task1, task2, task1_period, task2_period, system):
    ...
    trial_data1 = _test(
        model_path, model_file, options, env=env_dict[task1], noise=False
    )
    trial_data2 = _test(
        model_path, model_file, options, env=env_dict[task2], noise=False
    )
    ...
    task1_projected = pca_3d.transform(
        trial_data1_h_epoch.reshape((-1, trial_data1_h_epoch.shape[-1]))
    ).reshape((C_1, T_1, 3))
    task2_projected = pca_3d.transform(
    ...
    for c in range(options["batch_size"]):
        ...
        ax.scatter(
            task1_projected[c, 0, 0],
            task1_projected[c, 0, 1],
            task1_projected[c, 0, 2],
            s=150,
            marker="^",
            color=colors[7],
        )
        ax.scatter(
            task1_projected[c, -1, 0],
            task1_projected[c, -1, 1],
            task1_projected[c, -1, 2],
            s=150,
            marker="X",
            color=colors[7],
        )
        ...
        ax.scatter(
            task2_projected[c, 0, 0],
            task2_projected[c, 0, 1],
            task2_projected[c, 0, 2],
            s=150,
            marker="^",
            color="black",
        )
        ax.scatter(
            task2_projected[c, -1, 0],
            task2_projected[c, -1, 1],
            task2_projected[c, -1, 2],
            s=150,
            marker="X",
            color="black",
        )
        ...
        save_fig(
            os.path.join(
                exp_path, f"{task1}_{task2}", f"{system}_{task1}_{task2}_cond_{c}_pcs"
            ),
            eps=True,
        )

        fig, ax = ax_3d_no_grid()
        mtx1, mtx2, _ = scipy.spatial.procrustes(
            task1_projected[c, :], task2_projected[c, :]
        )
        ...
        ax.scatter(
            mtx1[0, 0], mtx1[0, 1], mtx1[0, 2], s=150, marker="^", color=colors[7]
        )
        ax.scatter(
            mtx1[-1, 0], mtx1[-1, 1], mtx1[-1, 2], s=150, marker="X", color=colors[7]
        )
        ...
        ax.scatter(
            mtx2[-1, 0], mtx2[-1, 1], mtx2[-1, 2], s=150, marker="X", color="black"
        )
        ...
        save_fig(
            os.path.join(
                exp_path,
                f"{task1}_{task2}",
                f"{system}_{task1}_{task2}_cond_{c}_procrustes_pcs",
            ),
            eps=True,
        )
...
def stable_lda_extension(model_name, add_new_rule_inputs=False):
    _epoch_lda(
        model_name, "stable", "extension", add_new_rule_inputs=add_new_rule_inputs
    )


def delay_lda_extension(model_name, add_new_rule_inputs=False):
    _epoch_lda(
        model_name, "delay", "extension", add_new_rule_inputs=add_new_rule_inputs
    )


def movement_lda_extension(model_name, add_new_rule_inputs=False):
    _epoch_lda(
        model_name, "extension", "extension", add_new_rule_inputs=add_new_rule_inputs
    )
...
def muscle_two_task_pcs_sinusoid_fullcircleclk(model_name):
    _two_task_pcs(
        model_name, "DlySinusoid", "DlyFullCircleClk", "all", "second", "muscle_acts"
    )


def neural_two_task_pcs_halfcircleclk_halfcirclecclk(model_name):
    _two_task_pcs(
        model_name, "DlyHalfCircleClk", "DlyHalfCircleCClk", "all", "all", "h"
    )


def muscle_two_task_pcs_halfcircleclk_halfcirclecclk(model_name):
    _two_task_pcs(
        model_name, "DlyHalfCircleClk", "DlyHalfCircleCClk", "all", "all", "muscle_acts"
    )
...
def _plot_bar(
    ...
    for l, combination in enumerate(combinations):
        ...
        if (
            combination_labels[l] == "extension_tasks"
...
def _trajectory_alignment(model_name):
    ...
    for env in env_dict:
        trial_data = _test(
            model_path, model_file, options, env=env_dict[env], noise=False
        )

        if (
            env == "DlyFullReach"
            or env == "DlyFullCircleClk"
            or env == "DlyFullCircleCClk"
            or env == "DlyFigure8"
            or env == "DlyFigure8Inv"
        ):
            halfway = int(
                (
            ...
            trial_data_h[env + "1"] = trial_data["h"][
                :, trial_data["epoch_bounds"]["movement"][0] : halfway
            ]
            trial_data_muscle[env + "1"] = trial_data["muscle_acts"][
                :, trial_data["epoch_bounds"]["movement"][0] : halfway
            ]

            trial_data_h[env + "2"] = trial_data["h"][
                :, halfway : trial_data["epoch_bounds"]["movement"][1]
            ]
            trial_data_muscle[env + "2"] = trial_data["muscle_acts"][
                :, halfway : trial_data["epoch_bounds"]["movement"][1]
            ]

        else:
            trial_data_h[env] = trial_data["h"][
                :,
                trial_data["epoch_bounds"]["movement"][0] : trial_data["epoch_bounds"][
                    "movement"
                ][1],
            ]
            trial_data_muscle[env] = trial_data["muscle_acts"][
                :,
                trial_data["epoch_bounds"]["movement"][0] : trial_data["epoch_bounds"][
                    "movement"
                ][1],
    ...
    for metric_label, metric in all_metrics.items():
        ...
        extension_retraction_vals = np.array(
            convert_motif_dict_to_list(extension_retraction_tasks, metric)
        )
        ...
        extension_retraction_labels = 3 * np.ones(
            (extension_retraction_vals.shape[0], 1)
        )

        X = np.concatenate(
            [subset_vals, extension_vals, retraction_vals, extension_retraction_vals]
        )
    ...
    ax.plot(
        x_labels,
        [metric_scores["neural_angles"], metric_scores["neural_shapes"]],
        linewidth=4,
        color="black",
    )
    ax.scatter(
    ...
    ax.plot(
        x_labels,
        [metric_scores["muscle_angles"], metric_scores["muscle_shapes"]],
        linewidth=4,
        color="black",
    )
    ax.scatter(
...
def dsa_similarity_matrix(model_name):
    ...
    for env in env_dict:
        trial_data = _test(
            model_path, model_file, options, env=env_dict[env], noise=True
        )
        ...
        if (
            ...
            retract = trial_data["h"][
                :, halfway : trial_data["epoch_bounds"]["movement"][1]
            ]

            pca_extend = PCA(n_components=12)
            extend_reduced = pca_extend.fit_transform(
                extend.reshape((-1, extend.shape[-1]))
            )
            extend_reduced = extend_reduced.reshape(
                (extend.shape[0], extend.shape[1], 12)
            )

            pca_retract = PCA(n_components=12)
            retract_reduced = pca_retract.fit_transform(
                retract.reshape((-1, retract.shape[-1]))
            )
            retract_reduced = retract_reduced.reshape(
                (retract.shape[0], retract.shape[1], 12)
            )
...
def procrustes_similarity_matrix(model_name):
    ...
    for env in env_dict:
        ...
        if (
            ...
            extend_reduced = extend_reduced.reshape(
                (extend.shape[0], extend.shape[1], 10)
            )
            ...
            retract_reduced = retract_reduced.reshape(
                (retract.shape[0], retract.shape[1], 10)
            )
...
def composite_input_init(model_name):
    ...
    for e, env in enumerate(env_dict_ext):
        composite_h = trial_data[env]["h"][
            :,
            trial_data[env]["epoch_bounds"]["delay"][0] : trial_data[env][
                "epoch_bounds"
            ]["delay"][1],
        ]
...
if __name__ == "__main__":
    ...
    if args.experiment == "trajectory_alignment_movement":
