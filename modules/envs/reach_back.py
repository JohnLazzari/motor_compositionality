...
class ReachBack(MotornetEnv):
    ...
    def reset(
        ...
        *,
        ...
        x_points_ext = fingertip[:, None, 0] + th.linspace(
            0, 1, steps=self.half_movement_time
        ).repeat(batch_size, 1) * (goal[:, None, 0] - fingertip[:, None, 0])
        y_points_ext = fingertip[:, None, 1] + th.linspace(
            0, 1, steps=self.half_movement_time
        ).repeat(batch_size, 1) * (goal[:, None, 1] - fingertip[:, None, 1])

        # Draw a line from goal to fingertip
        x_points_ret = goal[:, None, 0] + th.linspace(
            0, 1, steps=self.half_movement_time
        ).repeat(batch_size, 1) * (fingertip[:, None, 0] - goal[:, None, 0])
        y_points_ret = goal[:, None, 1] + th.linspace(
            0, 1, steps=self.half_movement_time
        ).repeat(batch_size, 1) * (fingertip[:, None, 1] - goal[:, None, 1])
