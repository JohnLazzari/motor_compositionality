...
def _test(config_path, model_path, model_file, batch_size=64, joint_state=None, env="RandomReach"):
    ...
    if joint_state == "center":
        joint_state = torch.tensor([(effector.pos_lower_bound[0] + effector.pos_upper_bound[0]) / 2, 
                                    (effector.pos_lower_bound[1] + effector.pos_upper_bound[1]) / 2, 0, 0]).unsqueeze(0).repeat(hp["batch_size"], 1)
